...
import { Wrench, Server, Shield, Activity, Sparkles, Zap, TrendingUp } from 'lucide-react';
import Link from 'next/link';
import type { Tool, MCPServer, AuthStatus } from '@/lib/arcade-client';
...
export default function DashboardPage() {
    ...
    async function loadDashboard() {
      try {
        ...
        const [toolsRes, serversRes, authRes] = await Promise.all([
          arcadeClient.listTools({ per_page: 1 }), // Only need count, not all tools
        ...
        // Fetch full tools list for display separately
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      {/* Elegant Header */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-purple-600/90 via-orange-500/90 to-purple-700/90 px-8 py-6 text-white shadow-xl backdrop-blur">
      ...
      <div>
              ...
              <p className="text-sm text-purple-100 opacity-90">
                Composable Dashboards Powered by Arcade
              </p>
            </div>
          </div>
      ...
      {/* Refined Analytics Grid */}
        ...
        <div 
          ...
          onClick={() => setExpandedGraph({
            type: 'line',
            title: 'Tool Executions This Week - Detailed View',
            data: [45, 52, 48, 65, 72, 68, 85]
          })}
        >
          <LineChartSimple 
            data={[45, 52, 48, 65, 72, 68, 85]}
            title="Tool Executions This Week"
          />
        </div>
        <div 
          ...
          onClick={() => setExpandedGraph({
            type: 'donut',
            title: 'Tools by Category - Detailed Breakdown',
            data: [
              { label: 'Developer', value: 8, color: '#8b5cf6' },
              { label: 'Productivity', value: 6, color: '#f97316' },
              { label: 'Communication', value: 5, color: '#10b981' },
              { label: 'Other', value: 3, color: '#3b82f6' },
            ]
          })}
        >
          <DonutChart
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div 
          ...
          onClick={() => setExpandedGraph({
            type: 'bar',
            title: 'Top Tools by Usage - Full Statistics',
            data: [
              { label: 'GitHub', value: 245, color: 'bg-gradient-to-r from-purple-500 to-purple-600' },
              { label: 'Slack', value: 189, color: 'bg-gradient-to-r from-orange-500 to-orange-600' },
              { label: 'Google', value: 167, color: 'bg-gradient-to-r from-blue-500 to-blue-600' },
              { label: 'Notion', value: 123, color: 'bg-gradient-to-r from-emerald-500 to-emerald-600' },
            ]
          })}
        >
          <BarChartSimple
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">12 tools ready to use</p>
          </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">9 active servers</p>
          </div>
